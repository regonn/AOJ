...
fn is_same_dice(dice1: &Vec<u8>, dice2: &Vec<u8>) -> bool {
...
fn main() {
    let mut dices: Vec<Vec<u8>> = vec![];
    let dice_count: u8 = read();
    let mut same_flag: bool = false;
    ...
    for _ in 1..dice_count {
        let mut temp_dice: Vec<u8> = vec![];
        for _ in 0..6 {
            let number: u8 = read();
            temp_dice.push(number);
        }
        dices.push(temp_dice)
    }

    let dices_length: usize = dices.len();

    for dice1_index in 0..dices_length {
        for dice2_index in (dice1_index + 1)..dices_length {
            let dice1 = &dices[dice1_index];
            let dice2 = &dices[dice2_index];
